...
<html>
	<head>
		...
		<script>
			function change(identity,status,i,order_id){
				...
				if(status==1){
					location="check.php?order_id="+order_id+"&action=send";
				}
			}
	...
	<?php
		...
		$sql = "select * from order_form,goods_information where goods_from='$user'";
		if(!empty($get)){
			if($_GET['status']=="5"){
				$sql.=" and status=0";
			}
		...
		$sql.=" and order_form.goods_id=goods_information.goods_id order by time desc";
...
ttt;
		...
		while($rows = mysqli_fetch_assoc($result)){
			...
			if($rows['status']=="0"){
				...
				echo ">已付款，等待发货</button><br><div class=\"al\">点击发货</div>";
			}
